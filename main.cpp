...
void search() {
  ...
  try {
    std::string title = readLine(readTitle);
    const LibraryItem *item = library.search(title);
    if (item!=nullptr) {
...
void indexOf() {
  ...
  std::cout << "Index: " << (index != -1 ? std::to_string(index) : "not found!") << std::endl;
}
...
int main() {
  ...
  menu addLibraryItemMenu = addMenu.subMenu("addLibraryItem(...)", addLibraryItem);
